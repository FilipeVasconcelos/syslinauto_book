...
\begin{criteria}{Critère du revers dans le plane de Black}
\textbf{Un système est stable en boucle fermée si lorsque parcourant 
...
\begin{criteria}{Critère du revers dans le plan de Bode}
\textbf{Un système est stable en boucle fermée si, pour la pulsation $\omega_{c0}$ telle que le module 
    de la fonction de transfert en boucle ouverte est égal à 1 (c.a.d $H_{BO}(\omega_{c0})=1$ ou encore
    $\SI{0}{\dB}$) }
\end{criteria}
...
Nous suivrons une présentation \og~graphique\fg du critère et de ce théorème à l'instar de~\cite{reg}. 
...
\end{center}
    ...
    le contour $\Gamma=F(\mathcal{C})$ fait alors $N=Z-P=2$ tours autour de l'origine dans le sens trigonométrique.
    \label{fig-contour_cauchy}}
\end{figure}



Remarquons que les tours sont comptés positivement dans le sens trigonométrique.
...
Bromwich\footnote{Thomas John I'Anson Bromwich (1875-1929), mathématicien anglais.}
...
\begin{axis}[
    ...
    \draw[dashed,thick] (axis cs:0,0) -- (axis cs:{1.4*cos(30)},{1.4*sin(30)}) 
        node[midway,below right,xshift=-1em] {$R\rightarrow\infty$};
...
\end{tikzpicture}
\end{center}
\caption{Contours de Nyquist/Bromwich}
\end{figure}
