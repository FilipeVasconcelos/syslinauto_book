...
%\begin{figure}
    %\includegraphics[width=0.8\textwidth]{scilab/}
%    \caption{Réponse impulsionnelle d'un système du second ordre}
%\end{figure}
...
La fonction de transfert en boucle fermée $H_{BF}$ est donnée par la relation de Black:
$$
...
$$
H_{BF}=\dfrac{1}{1+p}\dfrac{1}{1+\dfrac{1}{(1+p)^2}}=\dfrac{1+p}{1+(1+p)^2}=\dfrac{1+p}{p^2+2p+2}
$$

Avec l'opération \verb?feedback?, il est aisé de déterminer cette fonction de transfert 
en boucle fermée en quelques instructions.
\begin{Scilabcode}
--> p=%s
--> H=syslin('c',1,1+p);
--> R=syslin('c',1,1+p);
...
\raisebox{-\mydepth}{
    \fbox{\includegraphics[height=\myheight]{fig/demarrage.eps}}
}

Pour arrêter une simulation : cliquer sur l'icône : 
\raisebox{-\mydepth}{
    \fbox{\includegraphics[height=\myheight]{fig/stop.eps}}
}
