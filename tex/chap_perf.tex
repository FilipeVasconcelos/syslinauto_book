...
\begin{tikzpicture}
    ...
    \draw[dvtr] (axis cs:3*\t,0) node[below] {$t_{5\%}$} -- (axis cs:3*\t,\mc);
    \node[right] at (axis cs:6,\kk) {$s(\infty)$};
    \draw[dtb,{Latex[length=1.25mm]}-{Latex[length=1.25mm]}](axis cs:-0.25,\mc) 
    -- node[midway,left] {$\pm5\%$} (axis cs:-0.25,\pc);
    \draw[fill=red,red] (axis cs:3*\t,\mc) circle (1.5pt);
    \end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
    ...
    \node[right] at (axis cs:\xm,\kk) {$s(\infty)$}  ;
    \draw[dvtr] (axis cs:\tr,0) node[below] {$t_{5\%}$} -- (axis cs:\tr,\pc);
    \draw[dtb,{Latex[length=1.25mm]}-{Latex[length=1.25mm]}](axis cs:-0.25,\mc) 
    -- node[midway,left] {$\pm5\%$} (axis cs:-0.25,\pc);
    \draw[fill=red,red] (axis cs:\tr,\pc) circle (1.5pt);
    \end{axis}
\end{tikzpicture}
...
pour passer de 10\% à 90\% de la valeur finale.\footnote{Il existe une autre 
définition du temps de montée moins courante. Pour celle-ci le temps de montée 
correspond à la durée au bout de laquelle la réponse passe pour la première fois 
par la valeur finale. Cependant, l'inconvénient est que pour certaine réponse 
la valeur finale n'est jamais atteinte en un temps fini.}
...
\begin{tikzpicture}
    ...
    \draw[dtb] (axis cs:0,\mc) node[left] {10\%}-- (axis cs:\xm,\mc);
    \draw[dtb] (axis cs:0,\pc) node[left] {90\%}-- (axis cs:\xm,\pc);
    \draw[dvtr] (axis cs:\tu,0) node[below] {$t_1$} -- (axis cs:\tu,\mc);
    \draw[dvtr] (axis cs:\td,0) node[below] {$t_2$} -- (axis cs:\td,\pc);
    \node[right] at (axis cs:\xm,\kk) {$s(\infty)$};
    \draw[fill=red,red] (axis cs:\tu,\mc) circle (1.25pt);
    \draw[fill=red,red] (axis cs:\td,\pc) circle (1.25pt);
...
\begin{tikzpicture}
    ...
    \pgfmathsetmacro{\mc}{0.10*\kk}
    \pgfmathsetmacro{\pc}{0.90*\kk}
    \pgfmathsetmacro{\a}{0.45}               % amortissement xi
...
%\centering
%\begin{tikzpicture}
%\node at (0,0) {\includegraphics[width=\textwidth]{fig/fig_temps_de_reduit.eps}};
%\end{tikzpicture}
...
\begin{tikzpicture}
    ...
    [   legend style={draw=none,font=\normalsize},
        ...
        \addplot[mark=none,vtr,smooth]   table {python/tm.tab};
        \addplot[mark=none,vtn6g,smooth] table {python/tp.tab};
    \end{axis}
\end{tikzpicture}
\caption{Caractérisation de la rapidité d'un système du second ordre en
         fonction du taux d'amortissement $\xi$: par
         (bleu) le temps de réponse à 5\%, (vert) 
         le temps de pic $t_p$ et (rouge) le temps de montée $t_m$.
         d'amortissement $\xi$. Ces trois temps sont donnés en unité réduite
