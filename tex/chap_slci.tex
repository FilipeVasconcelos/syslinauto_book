...
\textsc{\romannumeral 20}\textsuperscript{e}~siècle, sous l'impulsion des travaux 
précurseurs de Claude Shannon\index{Shannon, Claude}, 
Warren McCulloch\index{McCulloch, Warren}, Norbert Wiener\index{Wiener, Norbert} 
...
\begin{tikzpicture}
		...
		\addplot [very thick,color=blue,domain=0:10, samples=201,unbounded coords=jump]{sin(3*deg(x))*exp(-x)+1};
...
\begin{tikzpicture}
		...
		\addplot [very thick,color=red,domain=0:10, samples=101,unbounded coords=jump]{0.5*x};
        \end{axis}
\end{tikzpicture}
...
\begin{tikzpicture}
	...
	ticks=none,
        ...
        \addplot [very thick,color=blue,domain=-2:0, samples=101,unbounded coords=jump]{0};
		\addplot [very thick,color=blue,domain=0:10, samples=201,unbounded coords=jump]{0.7*sin(3*deg(x))+1};
		\draw[dotted,very thick,blue] (axis cs:0,0) -- (axis cs:0,1);
        \end{axis}
\end{tikzpicture}
    {\tikzset{external/export=false}
...
\addplot[very thick,color=red,domain=-2:0, samples=101,unbounded coords=jump]{0};
%\addplot [very thick,color=red,domain=0:10, samples=101,unbounded coords=jump]{0.8*sin(3*deg(x))*exp(0.2*x)+1};
%\draw[dotted,very thick,red] (axis cs:0,0) -- (axis cs:0,1);
\end{axis}
\end{tikzpicture}
}
\caption{Trois exemples de signaux : (en bleu) stable (en rouge) instable}
\end{center}                
\end{figure}
