...
class Ftransfert():
    ...
    def evaluate(self,p,gain):
        ...
        if self.type == "function":
            ...
            return gain*self.num(p)/self.den(p)
    ...
    def nyquist(self,**kwargs):
        ...
        for kg,gain in enumerate(gains):
            ...
            for step in range(n):
                k=step-n//2
                om=k*dw
                h=self.evaluate(complex(0,om),gain)
                XNyq[kg].append(h.real)
                YNyq[kg].append(h.imag)
                if k == 0 : zeroPulse=om
            inftyPulse=n-1
...
if __name__ == "__main__":  
    ...
    print(repr(H0))
    print(str(H0))
    H0.nyquist(xlim=(-1,4),ylim=(-2,2),n=20000,dw=0.01,labels="$K=1$",color="tab:green")
    ...
    H2.nyquist(xlim=(-1,4),ylim=(-2,2),n=20000,dw=0.01,labels=["$K=1$","$K=2$","$K=3$","$K=4$"],color="tab:green",gains=[1,2,3])
    H3.nyquist(xlim=(-1,4),ylim=(-2,2),n=20000,dw=0.01,labels="$K=5$",color="tab:blue")
