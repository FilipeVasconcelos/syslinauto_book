...
Il existe donc un compromis entre précision et stabilité : augmenter $k_p$ 
...
\end{bequation}
%-------------------------------------------------------------------------------
Ce correcteur possède un pôle à l'origine. Il introduit donc une dynamique
...
\begin{itemize}
    \item \textbf{Module}
    \[
        \left|C_{\text{I}}(j\omega)\right|=\frac{k_i}{\omega}
        \quad\text{ou}\quad
        \left|C_{\text{I}}(j\omega)\right|_{dB}=20\log k_i-20\log \omega
    \]
    Le gain décroît avec une pente de $-20$ dB/décade.

    \item \textbf{Phase}
    \[
        \arg C_{\text{I}}(j\omega)=-90^\circ
    \]
    pour tout $\omega$. Le correcteur intégral introduit donc un retard de phase constant.
\end{itemize}
%-------------------------------------------------------------------------------
\begin{center}
...
La présence d'un pôle à l'origine augmente le type du système.
Il en résulte que l'erreur statique peut être annulée pour certaines
entrées, notamment pour une entrée en échelon.

Il existe donc un compromis entre précision et stabilité : le correcteur
intégral permet d'améliorer fortement la précision, mais peut ralentir
la réponse du système et favoriser l'apparition d'oscillations.
...
\og D filtré\fg, en effet l'action du terme $1+\tau p$ est celui d'un filtre
passe-bas.
%-------------------------------------------------------------------------------
\begin{center}
