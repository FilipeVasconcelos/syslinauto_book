...
\begin{itemize}
    ...
    \item $N$ le nombre de tours autour du point critique \textbf{(-1,0)} que fait le diagramme de Nyquist de $H_{BO}(p)$ 
          (équivalent au nombre de tours autour de l'origine de $1+H_{BO}(p)$)
    \item $P$ le nombre de pôles positifs de $1+H_{BO}(p)$ (équivalent au nombre de pôles de $H_{BO}(p)$)
\end{itemize}

D'après le principe de l'argument de Cauchy, appliqué $H_{BO}(p)$ avec le contour de Nyquist
(ou de Bromwich si nécessaire). Un système est stable en boucle fermée si $Z=P-N=0$.
