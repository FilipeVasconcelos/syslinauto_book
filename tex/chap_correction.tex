...
Un système peut présenter des dépassements (c.f \Cref{chap-model}), être précis, 
rapide (c.f \Cref{chap-perf}), stable (c.f \Cref{chap-stab}). Ces
...
l'écart
\[
    U(p)=\left(k_p^{(1)}+\dfrac{1}{\tau_i^{(1)} p}+\tau_d^{(1)} p\right)\epsilon(p)
\]
%-------------------------------------------------------------------------------
\begin{bequation}[ams align]
...
%\end{cases}
\begin{cases}
    k_p^{(1)}=k_p^{(2)}\left(1+\dfrac{\tau_d^{(2)}}{\tau_i^{(2)}}\right)\\[1.0em]
...
\[
\begin{cases}
    k_p^{(3)}=k_p^{(2)}\left(1+\dfrac{\tau_d^{(2)}}{\tau_i^{(2)}}\right)\\[1.0em]
    \tau_i^{(3)}=\tau_i^{(2)}+\tau_d^{(2)}\\[1.0em]
    \tau_d^{(3)}=\dfrac{\tau_i^{(2)}\tau_d^{(2)}}{\tau_i^{(2)}+\tau_d^{(2)}}
\end{cases}
\begin{cases}
    k_p^{(2)}=k_p^{(3)}\left(1+\dfrac{\tau_d^{(3)}}{\tau_i^{(3)}}\right)\\[1.0em]
