...
et fondamentaux pour la \textbf{modélisation} de l'automatique.
...
Ce chapitre nous permettra d'introduire l'outils mathématique 
indispensable que se doit de maîtriser l'automaticien à savoir 
\textbf{la transformée de Laplace.}
Celui-ci nous conduiera naturellement à la définition 
de la \textbf{fonction de transfert} d'un système et de ses différentes représentations.
...
d'automatique, elle possède cependant de nombreuses définitions.
...
C'est essentiellement cette dernière qui nous sera la plus utile dans
le cadre de ce cours. Un système sera alors considéré comme 
...
modélisés par des fonctions continus d'une variable $t$ de temps.
...
\begin{itemize}
    ...
    Schématiquement, on exprime ce principe de la façon suivante pour 
    ...
    \begin{tikzpicture}
        \begin{scope}[local bounding box=scope1]
            ...
            \sbRelier[$s(t)$]{B1}{S1}
        \end{scope}
        \begin{scope}[shift={($(scope1.east)+(1,0)$)}]
            \sbEntree{E3}
    ...
    \end{tikzpicture}
    \end{center}
    avec $\alpha\ \text{et}\ \beta\in\mathbb{R}$.	
\end{itemize}
...
\begin{itemize}
    \item \emph{Principe de causalité :}
        ...
        $$\forall t\quad e(t)=0\Rightarrow s(t)=0$$  
\end{itemize}
...
Un système invariant $\Sigma$, à l'aide d'un schéma-bloc simple est représenté ci-dessous 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Un système possédant les différentes propriétés précédentes est
dit linéaire continu et invariant ou \gls{slci}.
L'entrée et la sortie d'un \gls{slci} sont reliés par une 
équation différentielle à coefficients constants que l'on peut écrire dans le cas le
plus générale par la relation suivante :
...
de la physique d'un problème donnée (mécanique, électronique, optique \ldots).
Par exemple, dans le cas d'un problème de mécanique, 
la relation fondamentale de la dynamique est 
une source importante d'équations différentielles (appelées équations du mouvement). 
En électronique, ce sont les lois des noeuds et mailles qui 
permettront d'écrire de telles équations. 

\subsubsection{Mise en équation : système masse-ressort}

On considère un système mécanique constitué d'une masse $m$ en translation couplée avec un ressort de constante
de raideur $k_r$ et un amortisseur de coefficient de frottement visqueux $b$ (c.a.d proportionnelle à la vitesse). 
...
$$                                                                                                                            
m\devi{x(t)}{2}+b\devi{x(t)}{}+k_rx(t)=F(t)                                                                                   
$$ 
...
On identifie la forme de l'\cref{eq-difflci} avec $s(t)=x(t)$, $e(t)=F(t)$, 
$n=$2, $m=0$, $a_2=m$, $a_1=b$, $a_0=k_r$ et $b_0=$1.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%\subsubsection{Exemple d'équation différentielles à coefficients constants}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Un corps soumis à la gravité et à la résistance de l'air peut être considéré 
%comme un \gls{slci} pour lequel la sortie est la vitesse $v(t)$ à l'instant $t$
%après l'un avoir fourni une vitesse initiale $e(t)$ (ici soit une impulsion ou 


%On s'interresse à des systèmes très simples qui peuvent 
%\subsubsection{\ldots mécanique (chute libre)}

%\subsubsection{\ldots électronique}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Un signal est une variation d'une grandeur qui porte l'information de la 
sollicitation et de la réponse d'un système. Les signaux 
sont modélisés mathématiquement par des fonctions. 

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Types de signaux}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Il existe différents types de signaux: 
\begin{itemize}
    \item en temps continu (ou analogique).
    Ce signal sera modélisé par une fonction continue du temps. Formellement, par une fonction $s$ telle que :
        \begin{align*}
        s : \mathbb{R}&\rightarrow\mathbb{R} \\  
        t&\rightarrow s(t) 
        \end{align*}    
    \item en temps discret/échantillonné (ou numérique).
        \begin{align*}
...
seront traités lors d'un cours avancé d'automatique du cycle ingénieur.
...
$\omega=2\pi f$.
%L'analyse de Fourier est un outil fondamentale de 
%l'étude des signaux périodiques. Dans ce cours, nous
%aborderons 
{
\begin{figure}[htb]
\centering
...
\clearpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{Signaux usuels rencontrés\ldots}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Dans la pratique, l'impulsion de Dirac est utilisée pour caractériser le 
système. En effet la réponse impulsionnelle est caractéristique d'un
système linéaire. 
...
Puisque tous les signaux que nous rencontrerons sont des signaux causaux. Tout signaux $s(t)$
est modélisé par le produit d'une fonction $f(t)$ et de la fonction échelon-unité $u(t)$. Autrement dit,
pour rendre une fonction causale, il suffit de la multiplier par la fonction échelon-unité.
...
$$
La réponse d'un système à une rampe ne possède pas de nom communément 
...
et une sortie d'un système linéaire.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
La Transformée de Laplace ($\mathscr{L}$) d'une fonction causale $s$ (s pour signal) 
...
\end{bequation}
valable que si $pS(p)$ a tous ses pôles à partie réelle 
strictement négative, autrement dit que le signal soit stable.
