\begin{landscape}
    \newcommand{\mysize}{\footnotesize}
    \captionsetup{width=1.4\linewidth}
    \small
    \centering
    \captionof{table}{Réponses temporelles d'un système du 2nd ordre 
                      pour les différents régimes.}
    \setlength{\ltmp}{0.1\linewidth}
    \setlength{\ldtmp}{0.2\linewidth}
    \setlength{\lctmp}{0.23\linewidth}
    \ra{0.001}
    \begin{tabular}{@{}P{\ltmp}P{\ldtmp}P{\ldtmp}P{\ldtmp}@{}}%
    \toprule
    Réponse   &
    Régime apérodique        ($\xi>1$)  & 
    Régime critique ($\xi=1$) & 
    Régime pseudo-périodique ($0<\xi<1$) \\
    \midrule
    Réponse impulsionnelle & 
    \raisebox{-.5\height}{\resizebox{\lctmp}{!}{\includegraphics[width=0.5\textwidth]{figtikz/1-1.pdf}}} 
    {\mysize 
    \[
    s(t)=\dfrac{1}{\tau_1-\tau_2}
         \left(e^{-\frac{t}{\tau_1}}-e^{-\frac{t}{\tau_2}}\right)
    \]
    } &
    \raisebox{-.5\height}{\resizebox{\lctmp}{!}{\includegraphics[width=0.5\textwidth]{figtikz/1-2.pdf}}} 
    {\mysize 
        \[ 
    s(t)=\dfrac{t}{\tau^2}e^{-\frac{t}{\tau}}
        \]} &  
    \raisebox{-.5\height}{\resizebox{\lctmp}{!}{\includegraphics[width=0.5\textwidth]{figtikz/1-3.pdf}}} 
    {\mysize 
        \[
    s(t)=\dfrac{\omega_d}{1-\xi^2}e^{-\xi\omega_0 t}\sin{\omega_d t} 
        \]} \\
    \midrule
    Réponse indicielle &  
    \raisebox{-.5\height}{\resizebox{\lctmp}{!}{\includegraphics[width=0.5\textwidth]{figtikz/2-1.pdf}}} 
    {\mysize 
        \[
    s(t)=1+\dfrac{1}{\tau_1-\tau_2}\left(\tau_2e^{-\frac{t}{\tau_2}}
                    -\tau_1e^{-\frac{t}{\tau_1}}\right)
        \]} &  
    \raisebox{-.5\height}{\resizebox{\lctmp}{!}{\includegraphics[width=0.5\textwidth]{figtikz/2-2.pdf}}} 
    {\mysize 
        \[
    s(t)=1-e^{-\frac{t}{\tau}}-\dfrac{t}{\tau}e^{-\frac{t}{\tau}}
        \]} &  
    \raisebox{-.5\height}{\resizebox{\lctmp}{!}{\includegraphics[width=0.5\textwidth]{figtikz/2-3.pdf}}} 
    {\mysize 
        \[
    s(t) = 1 - \dfrac{e^{-\xi\omega_0 t}}{\sqrt{1-\xi^2}}\sin{(\omega_d t+\phi)}
        \]}\\
    \bottomrule
    \end{tabular}
    \begin{minipage}{0.9\linewidth}
    \noindent
    \scriptsize
    \textbf{Paramètres : 
    (pour tous) $K=1$, $E_0=1$ 
    (apériodique) $\xi=2$, $\omega_0=1$ (c.-à-d. $\tau_1=3.73$ 
                                              et $\tau_2=0.26$) 
    (critique) $\xi=1$, $\omega_0=1$ (c.-à-d. $\tau=1$) 
    (pseudo-périodique) $\xi=0.3$ et $\omega_0=1$}
    \end{minipage}
\end{landscape}
\captionsetup{width=\linewidth}
