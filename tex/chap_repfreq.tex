...
Cette relation exprime que \textbf{l'excitation d'un {\protect{\gls{slci}}}~par une entrée sinuso\"idale donne 
...
Considérons un \gls{slci}~définit par une fonction de transfert $H(p)$ du premier ordre (\Cref{eq-ft1er}) de forme canonique:
$$
H(p)=\dfrac{1}{1+p}
$$
...
\begin{table}
    ...
    \begin{tabular}{M{2.0cm}M{2.0cm}M{2.0cm}M{2.0cm}}
        \hhline{====}
        $\omega \si{[\radian\per\second]}$ & $\omega=1$    & $\omega=10$    & $\omega=100$ \\[0em]
        \hline
...
\begin{itemize}
    ...
    \item et le diagramme de Black-Nichols\footnote{\index{Nichols, Nathaniel}Nathaniel B. Nichols, (1914–1997) ingénieur américain.}. 
\end{itemize}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Un diagramme de Bode\footnote{\index{Bode, Hendrik}Hendrik Wade Bode (1905-1982), ingénieur, chercheur 
...
\caption{Représentation schématique d'un diagramme de Bode. Le gain en décibel et la phase 
    ...
    sur deux repères distincts.\label{fig-sche_bode}}
\end{figure}
...
Un diagramme de Nyquist\footnote{\index{Nyquist, Harry}Harry Nyquist (1889-1976), électronicien, ingénieur américain.} 
...
\end{center}
    ...
    symmétrique par rapport à l'axe des réels.\label{fig-sche_nyquist}}
\end{figure}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
Le diagramme de Black-Nichols\footnote{Il également simplement appelé diagramme de Black.} 
...
\caption{Représentation schématique d'un diagramme de Black. Le gain et la phase 
    ...
    pour différentes valeurs de la pulsation $\omega$ de 0 à $\infty$.\label{fig-sche_black}}
\end{figure}
...
La fonction de transfert d'un retard pur 
est de la forme $H(\jw)=e^{-\jtw}$,
le gain est donc simplement donné par
$$
G(\omega)=|H(\jw)|=1
$$
...
\end{tikzpicture}
    ...
    pulsations et le déphasage est monotone décroissant en fonction de la pulsation\label{fig-bode_retard_1}.}
\end{figure}
...
\caption{Diagramme de Bode d'un système du premier ordre 
    ...
    possible de lire un gain de -3 dB et une phase de -45\si{\degree} à la fréquence de coupure.\label{fig-bode_1er_3}}
\end{figure}
...
Il est notamment toujours possible d'écrire une fonction de transfert sous la forme d'un produit de gains purs, 
...
\begin{table}[!h]
    \resizebox{\linewidth}{!}{
    \begin{tabular}{M{1cm}M{2.5cm}M{2.5cm}M{2.5cm}M{2.5cm}M{2.5cm}}
    \hhline{======}
    & $\omega\ll\omega_1$ 
    ...
    \hhline{======}
    $G_{dB}(\omega)$ (pente) & 0(40dB)       &-20dB/décade    & -20dB/décade    & +40dB/décade & -20dB/décade \\[0em]
    \hline
        $\phi(\omega)$       & 0\si{\degree} &-90\si{\degree} & -90\si{\degree} & +180\degree  & -90\degree   \\[0em]
    \hhline{======}
    $G_{dB}(\omega)$ total   & 0(40dB)       &-20dB/décade    & -40dB/décade    & 0(-20dB)     & -20dB/décade \\[0em]
    \hline
    $\phi(\omega)$   total   & 0\degree      &-90\degree      & -180\degree     & 0            & -90\degree   \\[0em]
    \hhline{======}
    \end{tabular}}
\end{table}
...
\caption{Effet d'un retard sur le diagramme de Nyquist d'un système du premier ordre. 
    ...
    une spirale.\label{fig-nyquist_effet_retard_1}}
\end{figure}
