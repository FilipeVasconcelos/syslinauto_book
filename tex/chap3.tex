...
\begin{center}
    ...
    \begin{tikzpicture}
        ...
        \end{axis}
    \end{tikzpicture}
\caption{Réponse harmonique (\Cref{eq-rh}) d'un système du premier ordre
...
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section[Représentation de la réponse harmonique]{Représentation graphique de la réponse harmonique}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Dans cette partie, nous présenterons trois types de représentations 
graphiques  que sont les:
\begin{itemize}
...
\begin{axis}
    ...
    \draw[blue] (axis cs:1.9,0) arc (0:119:1.9)    node[midway,yshift=-2.5em,xshift=-3.0em] {$\phi(\omega_2)$};
    \addplot [thick,domain=0:7.853981633974483,samples=100]({x*sin(deg(x))},{x*cos(deg(x))});
\end{axis}
\end{tikzpicture}
\end{center}
\caption{Représentation schématique d'un diagramme de Nyquist. Le nombre complexe 
    $H(\jw)$ est représenté dans le plan complexe pour différentes valeurs de la 
    pulsation $\omega$ de 0 à $\infty$.\label{fig-nyquist}}
\end{figure}
...
Il est nottament possible d'écrire une fonction de transfert sous la forme d'un produit gain pur, d'intégrateur, de 
...
%    xmax=+10
%    ]     
%    \node [below]       at (axis cs:  6, 0)   {$\forall\omega$};                
%    \node [above,blue]       at (axis cs:  6, 0)   {$K$};                
%    \addplot [blue, mark = *] coordinates {(6, 0)} {};                          
%\end{axis}
%\end{tikzpicture}
%\end{center}
%\caption{Diagramme de Nyquist d'un gain pur. Le nombre complexe 
%    $H(\jw)$ est représenté par un point sur l'axe des réels à la valeur $K$.\label{fig-nyquist}}
%\end{figure}
...
\begin{axis}
    ...
    \addplot[blue,thick,domain=0.01:0.2,samples=50,-latex,unbounded coords=jump] (0,{-1/x});
    \addplot[blue,thick,domain=0.1:100,samples=50,unbounded coords=jump] (0,{-1/x});
    \addplot [dashed,blue,thick,domain=-100:-0.2,-latex,samples=100,unbounded coords=jump](0,{-1/x});
...
\caption{Diagramme de Nyquist d'un intégrateur pur. Le lieu de Nyquist 
    ...
    imaginaires purs négatifs.\label{fig-nyquist}}
\end{figure}
...
%    clip=false,
    ...
    \addplot [blue, mark = *] coordinates {(0, 0)} {};                          
    \addplot [blue,thick,domain=0:5,samples=50,-latex](0,{x}); 
    \addplot [blue,thick,domain=4:10,samples=50](0,{x}); 
    \addplot [dashed,blue,thick,domain=-10:-5,samples=50,-latex](0,{x}); 
...
\caption{Diagramme de Nyquist d'un dérivateur pur. Le lieu de Nyquist 
    est représenté par une demi droite sur l'axe des nombres 
    imaginaires purs positifs.\label{fig-nyquist}}
\end{figure}
...
%    ylabel style={left},                                                            
    ...
    \addplot [blue,thick,domain=0:1.1,samples=100,-latex]({1/(1+x*x)},{-x/(1+x*x)}); 
    ...
    \addplot [dashed,blue,thick,domain=-5:-0.5,samples=100,-latex]({1/(1+x*x)},{-x/(1+x*x)}); 
    \addplot [dashed,blue,thick,domain=-0.5:0,samples=100]({1/(1+x*x)},{-x/(1+x*x)}); 

    \node [above left] at (axis cs:  0, 0)  {$\omega\rightarrow-\infty$};         
    \node [below left] at (axis cs:  0, 0)  {$\omega\rightarrow+\infty$};         
...
\begin{axis}                                                                        
    ...
    \tikzstyle{lieua}=[blue,thick,samples=150,-latex]
    \tikzstyle{lieu}=[blue,thick,samples=150]
    \tikzstyle{lieua_c}=[dashed,blue,thick,samples=150,-latex]
    ...
    \node [above right] at (axis cs:  0, 0)  {$\omega\rightarrow-\infty$};
    \node [right]       at (axis cs:  1, 0)   {$\omega=0$};       
    \node [below right] at (axis cs:  0, 0)  {$\omega\rightarrow+\infty$};
    \addplot [blue, mark = *] coordinates {(0, 0)} {};                  
    \addplot [blue, mark = *] coordinates {(1, 0)} {};                  
\end{axis}                                                                          
...
\begin{axis}                                                                        
    ...
    \tikzstyle{lieua}=[thick,samples=100,-latex]
    \tikzstyle{lieu}=[thick,samples=100]
    \tikzstyle{lieua_c}=[dashed,thick,samples=100,-latex]
...
\begin{axis}                                                                        
    ...
    \tikzstyle{lieua}=[thick,samples=200,-latex]
    \tikzstyle{lieu}=[thick,samples=200]
    \tikzstyle{lieua_c}=[dashed,thick,samples=200,-latex]
    ...
    \addplot+[lieu, domain=2:3]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieu, domain=3:5]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieu,domain=5:8]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieu,domain=8:20]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieu,domain=20:100]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
                                                                                                                              
    \addplot+[lieu_c,domain=-100:-20]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieu_c,domain=-20:-8]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieu_c,domain=-8:-5]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieu_c,domain=-5:-3]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieu_c,domain=-3:-2]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieua_c,domain=-2:-1]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
    \addplot+[lieu_c,domain=-1.1:-0.5]({(1-x^2)/((1-x^2)^2+4*\xi^2*x^2)},{-(2*\xi*x)/((1-x^2)^2+4*\xi^2*x^2)});
