...
\begin{tikzpicture}
    ...
    \draw[dvtr] (axis cs:3*\t,0) node[below] {\small $t_{5\%}$} -- (axis cs:3*\t,\mc);
...
\begin{tikzpicture}
    ...
    \draw[dvtr] (axis cs:\tr,0) node[below] {\small $t_{5\%}$} -- (axis cs:\tr,\pc);
    \draw[dtb,{Latex[length=1.25mm]}-{Latex[length=1.25mm]}](axis cs:-0.25,\mc) 
    -- node[midway,left] {\small $\pm5\%$} (axis cs:-0.25,\pc);
...
\begin{bequation}[ams align]
t_{M}\omega_0=\dfrac{\pi-\arctan{\dfrac{\sqrt{1-\xi^2}}{\xi}}}{\sqrt{1-\xi^2}}\label{eq-tM}
\end{bequation}
%------------------------------------------------------------------------------
\begin{figure}[!b]
\centering
