...
%    \addplot[blue,thick,domain=1e2:1e4,samples=200]  ({-\dpp*atan2(\da*x,(1-\db*x*x))},{-\dk*10*log10((1-\db*x*x)*(1-\db*x*x)+\da*\da*x*x)});
    ...
    \draw[draw=none,fill=black] (axis cs:-180,0) circle (1.5pt) node[above] {\tiny$(-180,0\mathrm{dB})$};
...
Nous suivrons la présentation rencontrée dans~\cite{reg}
...
\end{center}
    ...
    le contour $\Gamma=F(\mathcal{C})$ fait alors $N=Z-P=2$ tours dans le sens trigonométrique.
