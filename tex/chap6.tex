...
\begin{itemize}
    ...
    \item \textbf{IV}: un quart de cercle de rayon $R\rightarrow\infty$ pour laquelle $p=Re^{j\theta}$ avec 
        $\theta\in[0,-\pi/2]$, symétrique de \textbf{II},
\end{itemize}

Seules les deux premières portions à considérer puisque les courbes images des 
portions \textbf{III} et \textbf{IV} peuvent être déterminés par symétrie par rapport 
à l'axe des réels des portions \textbf{I} et  \textbf{II} respectivement.

Déterminons la courbe image d'une fonction de transfert $H_{BO}(p)$
en boucle ouverte de ces deux portions. 
Notamment, l'image de la portion \textbf{I} est donné par $H_{BO}(I)=H_{BO}(\jw)$, ce qui correspond au lieu 
de Nyquist pour $\omega\in[0,\infty[$.
